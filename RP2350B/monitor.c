 ...
 * Pico 2 W (RP2350) port in ../RP2350/monitor.c -- see that file and this
 * repo's plan history for the full background. Two halves, same
 ...
 *      GPIO/transport-agnostic (talks to `buffer` and `f_*` only).
 *
 ...
 * property of the protocol, not something this port changes. (The Pico
 * 2 W port's QMI-CS1 SD driver needed __not_in_flash_func to avoid
 * deadlocking against its own open QMI transaction -- this port uses a
 * normal SPI1 peripheral instead, so that hazard doesn't apply here.)
 */
#include "monitor.h"
...
/* EXP_COMMAND_ROM_COPY_* state -- see those cases' own comments. Only
 * meaningful between a BEGIN and a FINISH; not reset on FINISH since a new
 * BEGIN always re-zeroes it before the first GET_BLOCK. */
...
static void DoCommand(uint8_t req, uint8_t buf[16][256]) {
    ...
    switch (req) {
        ...
        /* One-time ROM-to-SRAM bootstrap copy -- see PC_EXP.h's own
         * comment for the full sequence and rom/rom.asm for the LH5801
         * routine that drives this (staged into and run from inside the
         * 2K data window, since it has to finish before the 8800H-9FFFH
         * ROM region is populated). 6144 bytes / EXP_MAX_TRANSFER_LEN
         * (1024) = 6 clean blocks, no partial final block -- block N's
         * source is buffer[8 + N*4 .. 8 + N*4 + 3] (pages 8-31 already
         * hold the ROM image contiguously, per this file's own top
         * comment), copied into the payload region a page at a time.
         *
         * TWO open dependencies before this is real, not scaffolding:
         * (1) GreenPAK1's actual register-level I2C sequence for the
         *     ROM/SRAM-mux and write-enable flip-flops isn't designed yet
         *     (see greenpak_i2c.h's own comment, same gap as
         ...
         *     GET_BLOCK below will currently stage whatever buffer[8..31]
         *     happens to contain (zero-initialized static storage, i.e.
         *     all zero bytes) until that's wired up. Whether RP2350B
         *     should embed the ROM image the same compile-time way PSoC5
         *     does, or load it from the SD card at boot (it has full SD
        ...
        case EXP_COMMAND_ROM_COPY_GET_BLOCK: {
            ...
            romCopyBlockIndex++;
            buf[EXP_LENGTH_PORT_PAGE][EXP_LENGTH_PORT_ADDRESS] =
                (uint8_t)(EXP_MAX_TRANSFER_LEN >> 8);
            buf[EXP_LENGTH_PORT_PAGE][EXP_LENGTH_PORT_ADDRESS + 1] =
                (uint8_t)(EXP_MAX_TRANSFER_LEN & 0xFF);
            WriteStatus(buf, EXP_STATUS_SUCCESS);
            break;
        }
        ...
        case EXP_COMMAND_SD_LIST_CHANNELS: {
            ...
            window[1] = (uint8_t)(count & 0xFF);
            {
                 ...
                 * abandoned 4K-window layout) -- computed here instead so it
                 * can't drift out of sync with the window size again. Dead
                 * code in practice: EXP_DIR_MAX_ENTRIES already caps count
                 * before this could ever be false. */
                if (summaryOffset + EXP_DIR_SUMMARY_LEN <=
                    (uint16_t)(EXP_INSTRUCTION_PAGE * 256 + EXP_INSTRUCTION_ADDRESS)) {
                    char summary[16];
                    uint8_t pos = 0;
